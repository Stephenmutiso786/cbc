<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="card p-5 space-y-4">
        ...
        @if($pendingSmsStatus === 'pending')<div class="rounded border border-yellow-200 bg-yellow-50 p-3 text-sm text-yellow-800">SMS order is awaiting M-Pesa confirmation. <button wire:click="checkSmsStatus" class="ml-2 font-semibold underline">Check status</button></div>@elseif($pendingSmsStatus === 'awaiting_allocation')<div class="rounded border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">M-Pesa payment is confirmed. Your order is awaiting SMS allocation because the platform provider capacity is currently insufficient. The platform administrator has been alerted.</div>@elseif($pendingSmsStatus === 'confirmed')<div class="rounded border border-green-200 bg-green-50 p-3 text-sm text-green-800">SMS payment confirmed and credits added to this school wallet.</div>@elseif($pendingSmsStatus === 'failed')<div class="rounded border border-red-200 bg-red-50 p-3 text-sm text-red-800">SMS payment did not complete.</div>@endif
        <div class="text-sm">Current SMS wallet: <strong>{{ number_format($school->sms_credits) }} credits</strong></div>
        @if($smsOrders->isNotEmpty())<div class="overflow-x-auto"><table class="min-w-full text-xs"><thead class="border-b text-left uppercase text-gray-500"><tr><th class="py-2">Order</th><th class="py-2">Units</th><th class="py-2">Amount</th><th class="py-2">Status</th><th class="py-2">Receipt</th></tr></thead><tbody>@foreach($smsOrders->take(5) as $order)<tr class="border-b"><td class="py-2">{{ $order->created_at->format('d M Y') }}</td><td class="py-2">{{ number_format($order->units) }}</td><td class="py-2">KSh {{ number_format($order->amount, 2) }}</td><td class="py-2 capitalize">{{ str_replace('_', ' ', $order->status) }}</td><td class="py-2 font-mono">{{ $order->mpesa_receipt_number }}</td></tr>@endforeach</tbody></table></div>@endif
    </div>
