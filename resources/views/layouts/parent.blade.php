...
<div class="max-w-4xl mx-auto py-6 px-4">
    ...
    <nav class="mb-6 flex flex-wrap gap-2">
        ...
        @foreach([['parent.dashboard','Dashboard'],['parent.progress.index','Progress'],['parent.fees.index','Fees'],['parent.notes.index','Notes'],['parent.exam-timetable','Exam Timetable'],['parent.newsletters','Newsletters'],['parent.notifications.index','Notifications'],['parent.support.index','Support']] as [$r,$l])
        @php($lockedFeature = $featureMap[$r] ?? null)
        ...
        <a href="{{ $featureLocked ? route('feature.upgrade', ['feature' => $lockedFeature]) : route($r) }}" class="flex items-center justify-between px-4 py-2 rounded-lg text-sm font-medium bg-white border hover:bg-green-50 text-gray-700 {{ $featureLocked ? 'border-amber-300 bg-amber-50' : '' }}" @if($featureLocked) title="Upgrade required to use {{ $l }}" @endif><span>{{ $l }}</span><span>@if($featureLocked)&#128274;@elseif($badgeModule === 'support')<livewire:notifications.module-notification-badge :module="$badgeModule" />@endif</span></a>
        @endforeach
