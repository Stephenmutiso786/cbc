...
<div class="min-h-screen">
        ...
        <nav class="min-h-0 flex-1 space-y-4 overflow-y-auto px-3 py-4 text-sm">
            @if($navigationUser?->hasRole('super-admin'))
                @foreach([
                    ...
                    'Commercial' => [['admin.plans.index', 'Plans & Feature Access'], ['admin.roles.index', 'Access & Permissions'], ['admin.invoices.index', 'Platform Invoices'], ['admin.sms.index', 'SMS Balance & Recharge'], ['admin.schools.index', 'SMS Credit Allocation'], ['admin.broadcasts.index', 'Broadcast Centre']],
            ...
            @foreach([
                ...
                'Academics' => [['admin.classes.index', 'Classes', 'manage curriculum'], ['admin.subjects.index', 'Subjects', 'manage curriculum'], ['admin.grades.index', 'Grade Management', 'manage curriculum'], ['admin.promotions.index', 'Promotions', 'manage promotions'], ['admin.assessment.index', 'Assessments', 'view assessments'], ['admin.exams.index', 'Exams', 'view exams'], ['admin.exams.archive', 'Past Exams Archive', 'view exams'], ['admin.notes.index', 'Learning Notes', 'view notes'], ['admin.timetable.index', 'Timetable', 'view timetable'], ['admin.exam-timetable.index', 'Exam Timetable', 'view timetable']],
                'Finance' => [['finance.payments.index', 'Fees and Payments', 'view fees'], ['finance.invoices.index', 'Invoices', 'view fees'], ['finance.reports.index', 'Finance Reports', 'view finance reports'], ['billing.index', 'Subscription & Billing', null], ['billing.invoices', 'Platform Invoices', null]],
                'Operations' => [['admin.inventory.index', 'Inventory', 'view inventory'], ['admin.sms.index', 'SMS Center', 'view notifications'], ['admin.notifications.index', 'Notifications', 'view notifications'], ['admin.newsletters.index', 'School Newsletters', 'manage newsletters'], ['admin.reports.index', 'Analytics and Reports', 'view analytics'], ['admin.support.index', 'Support Tickets', 'submit support tickets']],
                'Configuration' => [['admin.settings.index', 'School Settings', 'manage system settings'], ['admin.backups.index', 'Backups', 'manage system settings'], ['admin.user-accounts.index', 'User Accounts', 'manage users'], ['admin.risk.index', 'At-Risk Learners', 'view analytics'], ['admin.impersonate.index', 'Impersonate Users', 'manage users'], ['admin.academic-periods.index', 'Years and Terms', 'manage curriculum'], ['admin.roles.index', 'Roles and Permissions', 'manage roles'], ['admin.report-forms.index', 'Report Forms', 'view report cards'], ['admin.drive-store.index', 'Google Drive Store', 'view report cards'], ['admin.kemis.index', 'KEMIS Integration', 'sync kemis'], ['admin.schools.index', 'Manage Schools', '__super_admin__'], ['admin.plans.index', 'Manage Plans', '__super_admin__'], ['admin.platform-analytics.index', 'Platform Analytics', '__super_admin__'], ['admin.risk.platform', 'AI Risk Predictions', '__super_admin__'], ['admin.platform-finance.index', 'Platform Finance', '__super_admin__'], ['admin.broadcasts.index', 'Broadcast Center', '__super_admin__'], ['admin.id-cards.index', 'ID Cards & Certificates', 'view students'], ['admin.legal-policies.index', 'Legal Policies', 'manage legal policies'], ['admin.diagnostics.index', 'System Diagnostics', 'run diagnostics'], ['admin.system-logs.index', 'System Logs', '__super_admin__'], ['admin.impersonate.index', 'Impersonate User', '__super_admin__'], ['legal.terms', 'Terms and Conditions', null], ['legal.privacy', 'Privacy Policy', null]],
            ] as $section => $links)
                <div><p class="mb-1 px-4 text-[10px] font-bold uppercase tracking-widest text-green-300">{{ $section }}</p>@foreach($links as [$route, $label, $permission])@if($maySeeNavigationLink($permission))@php($lockedFeature = $featureMap[$route] ?? null)@php($featureLocked = $lockedFeature && !(($navigationUser?->school?->hasFeature($lockedFeature)) ?? true))@php($badgeModule = app(\App\Services\ModuleNotificationService::class)->moduleForRoute($route))<a href="{{ $featureLocked ? route('feature.upgrade', ['feature' => $lockedFeature]) : route($route) }}" class="flex items-center justify-between rounded-lg px-4 py-2.5 text-green-100 hover:bg-green-700 {{ $featureLocked ? 'bg-green-900/40' : '' }}" @if($featureLocked) title="Upgrade required to use {{ $label }}" @endif><span>{{ $label }}</span><span class="flex items-center gap-2">@if($featureLocked)<span aria-label="Upgrade required" class="text-xs text-green-200">&#128274;</span>@endif @if($badgeModule === 'support' && ! $featureLocked)@php($badgeCount = app(\App\Services\ModuleNotificationService::class)->count($badgeModule, $navigationUser?->id, $navigationUser?->hasRole('super-admin')))<span class="ml-2 inline-flex min-w-5 items-center justify-center rounded-full bg-red-600 px-1.5 py-0.5 text-[10px] font-bold leading-none text-white" @if($badgeCount < 1) hidden @endif>{{ $badgeCount > 99 ? '99+' : $badgeCount }}</span>@endif</span></a>@endif @endforeach</div>
            @endforeach
            @endif
        </nav>
    ...
    <div class="md:pl-64">
        ...
        <main class="min-w-0 overflow-x-hidden p-4 md:p-6">@include('layouts.partials.breadcrumbs') @yield('content') @isset($slot){{ $slot }}@endisset</main>
