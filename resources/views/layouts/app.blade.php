    ...
    <aside data-sidebar class="fixed inset-y-0 left-0 z-40 flex w-64 -translate-x-full flex-col bg-green-800 text-white shadow-lg transition-transform duration-300 md:translate-x-0">
        ...
        <nav class="min-h-0 flex-1 space-y-4 overflow-y-auto px-3 py-4 text-sm">
            @foreach([
                ...
                <div><p class="mb-1 px-4 text-[10px] font-bold uppercase tracking-widest text-green-300">{{ $section }}</p>@foreach($links as [$route, $label, $permission])@if($permission === '__super_admin__' ? auth()->user()->hasRole('super-admin') : ($permission === null || auth()->user()->can($permission)))@php($badgeModule = app(\App\Services\ModuleNotificationService::class)->moduleForRoute($route))<a href="{{ route($route) }}" class="flex items-center justify-between rounded-lg px-4 py-2.5 text-green-100 hover:bg-green-700"><span>{{ $label }}</span>@if($badgeModule)<livewire:notifications.module-notification-badge :module="$badgeModule" />@endif</a>@endif @endforeach</div>
            @endforeach
        </nav>
